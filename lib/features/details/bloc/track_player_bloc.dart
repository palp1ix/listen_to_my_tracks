...
class TrackPlayerBloc extends Bloc<TrackPlayerEvent, TrackPlayerState> {
  ...
  void _listenToPlayerStreams() {
    _statusSubscription = _audioPlayerService.statusStream.listen(
      (status) => add(_PlayerStatusChanged(status as PlayerStatus)),
    );
  ...
  Future<void> _onLoadTrack(LoadTrack event, Emitter<TrackPlayerState> emit) async {
  ...
  Future<void> _onPlayRequested(PlayRequested event, Emitter<TrackPlayerState> emit) async {
    await _audioPlayerService.play();
  }

  Future<void> _onPauseRequested(PauseRequested event, Emitter<TrackPlayerState> emit) async {
    await _audioPlayerService.pause();
  }

  Future<void> _onSeekRequested(SeekRequested event, Emitter<TrackPlayerState> emit) async {
    await _audioPlayerService.seek(event.position);
  }

  void _onPlayerStatusChanged(_PlayerStatusChanged event, Emitter<TrackPlayerState> emit) {
    emit(state.copyWith(status: event.status));
  }

  void _onPlayerPositionChanged(_PlayerPositionChanged event, Emitter<TrackPlayerState> emit) {
    emit(state.copyWith(currentPosition: event.position));
  }

  void _onPlayerDurationChanged(_PlayerDurationChanged event, Emitter<TrackPlayerState> emit) {
