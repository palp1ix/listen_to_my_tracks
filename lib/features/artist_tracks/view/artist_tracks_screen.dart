    ...
    return Scaffold(
      ...
      body: BlocBuilder<ArtistTracksBloc, ArtistTracksState>(
        ...
        builder: (context, state) {
          return switch (state) {
            ArtistTracksInitial() ||
            ArtistTracksLoading() => const _LoadingView(),
            ArtistTracksLoaded(:final tracks) => _LoadedTracksView(
              tracklist: tracks,
              artistId: state.artistId,
            ),
            ArtistTracksError(:final message) => _ErrorView(
              message,
              state.artistId,
            ),
          };
...
class _LoadedTracksView extends StatelessWidget {
  const _LoadedTracksView({required this.tracklist, required this.artistId});
  ...
  Widget build(BuildContext context) {
    if (tracklist.isEmpty) {
      return _ErrorView(
        'No availible tracks in the chart. Try to use VPN', artistId,
      );
    }
...
class _ErrorView extends StatefulWidget {
  const _ErrorView(this.errorMessage, this.artistId);
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              _artistTracksBloc.add(
                // Retry loading tracks for the same artist.
